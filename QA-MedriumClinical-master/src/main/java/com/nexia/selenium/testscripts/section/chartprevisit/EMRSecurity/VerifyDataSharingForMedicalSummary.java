...
public class VerifyDataSharingForMedicalSummary extends AbstractChartPreVisit {	

	@Test(groups = {"Approved","firefox", "iexplore", "safari", "default" }, description = "Test to validate that all users that mapped to the practices that share Medical summary will have access to medical summary")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void dataSharingForMedicalSummaryTest(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib providerData = new ChartPreVisitLib();
		providerData.workSheetName = "EMRSecurity";
		providerData.testCaseId = "TC_ES_006";
		providerData.fetchChartPreVisitTestData();
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		dataSharingForMedicalSummary(seleniumHost, seleniumPort, browser, webSite, userAccount, providerData);
	}

	/*************************************************************************************************** 
	 * @purpose        Test to validate that all users that mapped to the practices that share Medical summary will have access to medical summary 
	...
	public boolean dataSharingForMedicalSummary(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib providerData) throws Exception{
