...
public class VerifyBalanceOnBillingHistory extends AbstractSystemSettingsTest {
    ...
    public void verifyInsuranceBalance(String seleniumHost, int seleniumPort, String browser, String webSite, String userAccount) throws Exception {
    ...
    @Test(groups = { "AdvancedSmoke", "Regression", "Approved", "firefox", "iexplore", "safari", "default" }, description = "Test for verify hard copy check box")
    @Parameters({ "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount" })
    public void verifyPatientBalance(String seleniumHost, int seleniumPort, String browser, String webSite, String userAccount) throws Exception {
	SystemSettingsLib payerAdminData = new SystemSettingsLib();
	payerAdminData.workSheetName = "PayerAdmin";
	...
	try {
	    ...
	    if(payerAdminData.testCaseId.equals("TC_PA_010")){
		 ibalanceNumber=Float.parseFloat(getText(selenium, lblInsuranceBalance).replace('$',' ').replaceAll(",","").trim());
	    }
	    else{
		ibalanceNumber=Float.parseFloat(getText(selenium, lblPatientBalance).replace('$',' ').replaceAll(",","").trim());
	    }
	   ...
	   assertTrue(selectValueFromAjaxList(selenium, ajxRelationShip, payerData.relationshipStatus), "Could not select Relationship Status"
	    ...
	    //String payer=getValue(selenium,ajxBillPayer);
	    ...
	    String balance="";
	    balance=billing.savePostChargeEntryForBalanceCalculation(selenium, chargeEntryData,balance, userAccount);
	   ...
	   //assertTrue(goToBillingHistory(selenium,payerData),"Navigation Failed ", selenium, ClassName, MethodName);
	    ...
	    if(!verifyBalance(selenium,balance,ibalanceNumber,payerAdminData)){
		Assert.fail("Patient Balance verification failed");
	    }
    ...
    public boolean verifyBalance(Selenium selenium,String balance,float ibalanceNumber,SystemSettingsLib payerAdminData){
	boolean returnValue=true;
	float iChargeAmt,ibalanceNumber1,iBalance;
	iChargeAmt=Float.parseFloat(balance.trim());
	if(payerAdminData.testCaseId.equals("TC_PA_010")){
	    ibalanceNumber1=Float.parseFloat(getText(selenium, lblInsuranceBalance).replace('$',' ').replaceAll(",","").trim());
	    iBalance=iChargeAmt+ibalanceNumber;
	    
	    if(iBalance!=ibalanceNumber1){
		//Assert.fail("Balance Update Failed");
	    }
	}
	else{
	    ibalanceNumber1=Float.parseFloat(getText(selenium, lblPatientBalance).replace('$',' ').replaceAll(",","").trim());
	    iBalance=iChargeAmt+ibalanceNumber;
