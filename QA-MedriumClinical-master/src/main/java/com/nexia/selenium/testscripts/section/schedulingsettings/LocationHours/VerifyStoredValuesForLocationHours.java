...
public class VerifyStoredValuesForLocationHours extends AbstractSchedulingTest {
	...
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Verify locations on scheduling screen")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySaveLocationOnSchedulingWithBookingAllowed(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		SchedulingTestLib lhour = new SchedulingTestLib();
		lhour.workSheetName = "LocationHours";
	...
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Verify locations on scheduling screen")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySaveLocationOnSchedulingWithBookingNotAllowed(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		SchedulingTestLib lhour = new SchedulingTestLib();
		lhour.workSheetName = "LocationHours";
		lhour.testCaseId = "TC_LH_006";
		lhour.fetchSchedulingTestData();
	...
	public boolean verifyLocations(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib lhour) throws Exception{
		...
		try{
			...
			//--------------------------------------------------------------------//
			 ...
			 if(lhour.testCaseId.equals("TC_LH_006")){
				 ...
				 waitForPageLoad(selenium);
				 selectValueFromAjaxList(selenium,txtSearchBox,lhour.location1);
				 if(getText(selenium,lblLocationHours).contains("bookings allowed during holidays")){
						return returnValue;
					}
					 else{
						 returnValue=false;
			 ...
			 if(lhour.testCaseId.equals("TC_LH_007")){
				 ...
				 waitForPageLoad(selenium);
				 selectValueFromAjaxList(selenium,txtSearchBox,lhour.location1);
				 if(getText(selenium,lblLocationHours).contains("bookings not allowed during holidays")){
						return returnValue;
					}
					 else{
						 returnValue=false;
