...
public class VerifyAuditLogForTimeLinePrintSend extends AbstractAuditLog {
	
	@Test(groups = {"Review","firefox", "iexplore", "safari", "default" }, description = "Test for verify audit log In TimeLine print/send")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyAuditLogForTimeLinePrintSend(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		AuditLogLib timeLineData = new AuditLogLib();
		timeLineData.workSheetName = "AuditLogForTimeLine";
		timeLineData.testCaseId = "TC_TL_002";
	...
	/*************************************************************************************************** 
	* @purpose        To verify Audit Log for TimeLine print/send
	* @action 		  verify Audit Log for TimeLine print/send
	* @author         Aspire QA
	* @state          Developing
	* @useraccount    Both
	* @since  	      Oct 21, 2013
	***************************************************************************************************/
	...
	public boolean verifyPrintSendInTimeLine(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, AuditLogLib timeLineData) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-8: Verify time line print/ send logs in audit log			  //
			//--------------------------------------------------------------------//
			if(!verifyStoredValuesForTimeLinePrintSend(selenium,timeLineData, userAccount)){
	...
	public boolean verifyStoredValuesForTimeLinePrintSend(Selenium selenium,AuditLogLib auditData, String account){
