...
public class EditPlanPayersAdmin  extends AbstractSystemSettingsTest {
	...
	/**
	 * edit Plan PayerAdmin
	 * function to edit Plan PayerAdmin 
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	...
	*/
	...
	public boolean createPayerAdmin(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SystemSettingsLib payerAdminData) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-7: Edit the previously created Payers Plan		  					  //
			//--------------------------------------------------------------------//
			...
			assertTrue(click(selenium,"link=Edit"),"Could not click on Edit button; More details"+payerAdminData.toString(), selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			assertTrue(createPayersPlan(selenium,payerAdminData, userAccount),"Payers Plan Creation Failed", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			//--------------------------------------------------------------------//
			//  Step-8: Verify the Saved Payers Plan		  					  //
			//--------------------------------------------------------------------//
			assertTrue(verifyPayersPlanValuesInSummaryPage(selenium,payerAdminData, userAccount), 
					"Payer Admins details not saved proberly:"+payerAdminData.toString(), selenium, ClassName, MethodName);
