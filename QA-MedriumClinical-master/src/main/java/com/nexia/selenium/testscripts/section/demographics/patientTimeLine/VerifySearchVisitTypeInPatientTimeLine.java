...
public class VerifySearchVisitTypeInPatientTimeLine extends AbstractSchedulingTest{
	...
	public void searchWithVisitType(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount)throws Exception{
		...
		patientTimeLineData.testCaseId = "TC_PTL_001";
	...
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test For Verifying Location Details in Patient Time Line ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void searchWithLocation(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount)throws Exception{
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		SchedulingTestLib patientTimeLineData = new SchedulingTestLib();
		patientTimeLineData.workSheetName = "PatientTimeLine";
	...
	***************************************************************************************************/
	...
	public boolean verifySearchVisitType(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib patientTimeLineData) throws Exception{
		...
		try{
			...
			if(patientTimeLineData.testCaseId.equals("TC_PTL_001")){
				assertTrue(type(selenium,ajxSearchVisitOrResouce,patientTimeLineData.visitType ),"Could not search the visit type ", selenium , ClassName, MethodName);
				waitForPageLoad(selenium);
				selenium.keyPress(txtPatientSearch, "\\9");
				waitForPageLoad(selenium);
				
			}else if(patientTimeLineData.testCaseId.equals("TC_PTL_002")){
				
				assertTrue(type(selenium,ajxSearchVisitOrResouce, providerName),"Could not search the provider name ", selenium , ClassName, MethodName);
				waitForPageLoad(selenium);
				selenium.keyPress(txtPatientSearch, "\\9");
				waitForPageLoad(selenium);
			}else{
				assertTrue(type(selenium,ajxSearchVisitOrResouce, LocationName),"Could not search the provider name ", selenium , ClassName, MethodName);
				waitForPageLoad(selenium);
				selenium.keyPress(txtPatientSearch, "\\9");
				waitForPageLoad(selenium);
			}
