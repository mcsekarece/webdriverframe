...
public class VerifyOnlyThisRecurrenceSeries extends AbstractTaskManagement{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verifying only this Recurrence series")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void onlyThisRecurrenceSeriesSendTask(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		verifyOnlyThisRecurrence(seleniumHost, seleniumPort, browser, webSite, userAccount,createPatientTask);
	}
	
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verifying only this Recurrence series")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void onlyThisRecurrenceSeriesCancelTask(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		TaskManagementLib createPatientTask = new TaskManagementLib();
		createPatientTask.workSheetName = "createPatientTask";
		createPatientTask.testCaseId = "TC_CGT_020";
		createPatientTask.fetchTaskManagementTestData();
	...
	/*************************************************************************************************** 
	* @purpose        verify Only This Recurrence series
	* @action 		  verifying Only This Recurrence series
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	***************************************************************************************************/
	...
	public boolean verifyOnlyThisRecurrence(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, TaskManagementLib createPatientTask ) throws IOException{
		...
		try{
			...
			assertTrue(click(selenium,chckOnlyThisRecurrence),"could not click the only this recurrence radio button",selenium, ClassName, MethodName);
			waitForPageLoad(selenium);	
			
			if(createPatientTask.testCaseId.equals("TC_CGT_020")){
				assertTrue(click(selenium,btnSend),"Could not click the send button", selenium, ClassName, MethodName);
				waitForPageLoad(selenium);	
			}else if (createPatientTask.testCaseId.equals("TC_CGT_002")){
