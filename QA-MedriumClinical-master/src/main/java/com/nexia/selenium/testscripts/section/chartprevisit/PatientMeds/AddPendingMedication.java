...
public class AddPendingMedication extends AbstractChartPreVisit{
	@Test(groups = {"AdvancedSmoke","Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for restart a medication")
	...
	public void addPendingMedication(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		prescribeData.testCaseId = "TC_PM_085";
	...
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void addPendingMedicationForFreeText(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib prescribeData = new ChartPreVisitLib();
		prescribeData.workSheetName = "PrescribeMedi";
		prescribeData.testCaseId = "TC_PM_086";
		prescribeData.fetchChartPreVisitTestData();
	...
	//@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for restart a medication")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void addPendingMedicationForFreeForm(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib prescribeData = new ChartPreVisitLib();
		prescribeData.workSheetName = "PrescribeMedi";
		prescribeData.testCaseId = "TC_PM_087";
		prescribeData.fetchChartPreVisitTestData();
	...
	public boolean pendingMedication(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib prescribeData) throws Exception{
		...
		assertTrue(switchRole(selenium,prescribeData.switchRole), "Could not switch the role", selenium, ClassName, MethodName);
			...
			if(prescribeData.testCaseId.equals("TC_PM_087")){
			...
			waitForPageLoad(selenium);
			waitForPageLoad(selenium);
			assertTrue(isElementPresent(selenium, 	lnkGoToPending), "Blue box is not present", selenium, ClassName, MethodName);
								
			assertTrue(click(selenium,lnkMedicationTab),"Could not medication tab"+prescribeData.toString(), selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
				if(getText(selenium,lblPendingPrecribeName).toLowerCase().contains(prescribeData.prescribeCa.toLowerCase())
						||getText(selenium,lblPendingPrecribeName).contains(prescribeData.prescribeName)){
					return returnValue;
				}else
