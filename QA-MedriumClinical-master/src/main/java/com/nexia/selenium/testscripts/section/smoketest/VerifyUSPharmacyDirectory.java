...
public class VerifyUSPharmacyDirectory extends AbstractSmokeTest{

	@Test(groups = {"SmokeUS","RegressionUS","Approved","firefox", "iexplore", "safari", "default" }, description = "Search Pharmacy with a postal code")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void USPharmacyDirectory(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		SmokeTestLib searchData = new SmokeTestLib();
		searchData.workSheetName = "SearchPharmacy";
		searchData.testCaseId = "TC_SP_002";
		searchData.fetchSmokeTestData();
		SearchPharmacy(seleniumHost, seleniumPort, browser, webSite, searchData, userAccount);
	}	
	
	
	/**
	 * SearchPharmacy
	 * function for US Search Pharmacy
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	...
	public boolean SearchPharmacy(String seleniumHost, int seleniumPort,String browser, String webSite, SmokeTestLib searchData, String userAccount)  throws IOException{
		...
		try{
			...
			assertTrue(searchPatientNexiaForProviderHomePage_smoke(selenium,searchData.patientId),"could not find text", selenium, ClassName, MethodName);
	...
	public boolean verifySearchPharmacy(Selenium selenium , SmokeTestLib searchData) throws IOException{
		...
		assertTrue(selenium.isTextPresent("Select"),"could not find text", selenium, ClassName, MethodName);
		assertTrue(selenium.isTextPresent("Tel"),"could not find text", selenium, ClassName, MethodName);
		assertTrue(selenium.isTextPresent("Fax"),"could not find text", selenium, ClassName, MethodName);	
