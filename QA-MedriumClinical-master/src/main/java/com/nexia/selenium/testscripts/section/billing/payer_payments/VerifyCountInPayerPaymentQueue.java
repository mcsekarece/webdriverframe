...
public class VerifyCountInPayerPaymentQueue extends AbstractBilling {
    	
    @Test(groups = {"RegressionUS","Approved","firefox", "iexplore", "safari", "default" }, description = "Verify count in the payer payment queue")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyCountInPayerPaymentQueue(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		verifyCountInPayerPaymentQueue(seleniumHost, seleniumPort, browser, webSite, userAccount, billingData);
	}
    
    /***************************************************************************************************
	* @purpose        Verify count in the payer payment queue
	* @action 		  verifying count in the payer payment queue
	...
	* @since  	      Feb 15, 2014	
	***************************************************************************************************/
	public boolean verifyCountInPayerPaymentQueue(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, BillingLib billingData) throws IOException{
		...
		try{
			...
			int unPostedCount = Integer.parseInt(getText(selenium, lblUnpostedCount)); 
			...
			if(userAccount.equals(USAccount)){
				...
				//--------------------------------------------------------------------//
				//  Step-5: Verify count in the payer payment queue					  //
				//--------------------------------------------------------------------//
				int updatedUnPostedCount = Integer.parseInt(getText(selenium, lblUnpostedCount));
				
				if(unPostedCount == updatedUnPostedCount){
					Assert.fail("updation in conut verification is failed!!");
				}
			}
