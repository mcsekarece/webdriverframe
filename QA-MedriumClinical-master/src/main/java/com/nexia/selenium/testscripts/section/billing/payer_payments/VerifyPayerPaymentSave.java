...
public class VerifyPayerPaymentSave extends AbstractBilling {
    	
    @Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Verify payer payment save")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyPayerPaymentSave(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		verifyPayerPaymentSave(seleniumHost, seleniumPort, browser, webSite, userAccount, billingData);
	}

    /*************************************************************************************************** 
	* @purpose        Verify payer payment save
	* @action 		  verifying payer payment save
	...
	* @since  	      Feb 05, 2014	
	***************************************************************************************************/
	public boolean verifyPayerPaymentSave(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, BillingLib billingData) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-4: Verify save in payer payments 							  //
			//--------------------------------------------------------------------//
			...
			//--------------------------------------------------------------------//
			//  Step-5: Verify header names and saved payer payment in the queue  //
			//--------------------------------------------------------------------//
			if(!(getText(selenium, lblPayerNameUnpostQueue).equalsIgnoreCase("Payer Name"))){
				return false;
			}
			if(!(getText(selenium, lblCheckNumberUnpostQueue).equalsIgnoreCase("Check #"))){
				return false;
			}
			if(!(getText(selenium, lblEnteredAmtUnpostQueue).equalsIgnoreCase("Entered Amount"))){
				return false;
			}
			if(!(getText(selenium, lblCheckAmtUnpostQueue).equalsIgnoreCase("Check Amount"))){
				return false;
			}
			if(!(getText(selenium, lblStartedOnUnpostQueue).equalsIgnoreCase("Started on"))){
				return false;
			}
			if(!(getText(selenium, lblDepositDateUnpostQueue).equalsIgnoreCase("Deposit date"))){
				return false;
			}
