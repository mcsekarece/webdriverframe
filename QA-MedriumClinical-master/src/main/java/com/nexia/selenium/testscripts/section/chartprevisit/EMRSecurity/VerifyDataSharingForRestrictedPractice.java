...
public class VerifyDataSharingForRestrictedPractice extends AbstractChartPreVisit {	

	@Test(groups = {"Smoke","firefox", "iexplore", "safari", "default" }, description = "Test to validate that all users that mapped to the practices that are Restricted will have no access to medical summary and medical data")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void dataSharingForRestrictedPracticeTest(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib providerData = new ChartPreVisitLib();
		providerData.workSheetName = "EMRSecurity";
		providerData.testCaseId = "TC_ES_007";
		providerData.fetchChartPreVisitTestData();
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		dataSharingForRestricted(seleniumHost, seleniumPort, browser, webSite, userAccount, providerData);
	}

	/*************************************************************************************************** 
	 * @purpose        "Test to validate that all users that mapped to the practices that are Restricted will have no access to medical summary and medical data") 
	...
	public boolean dataSharingForRestricted(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib providerData) throws Exception{
		...
		try{
			...
			if(vitalInfo.contains("Last Recorded on")) {			
				Assert.fail("Vital text found");
		    }else {
		    	System.out.println("No Vitals data found");
		    }
			...
			if(isElementPresent(selenium, lblFirstPendingEncounter) || isElementPresent(selenium, lblFirstSignedEncounter)){
				Assert.fail("Found Encounter");				 
			}else {
				System.out.println("No encounter data found");
			}
