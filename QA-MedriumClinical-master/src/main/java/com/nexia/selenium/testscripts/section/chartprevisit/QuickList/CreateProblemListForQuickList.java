/*************************************************************************************************** 
* @purpose 			This testcase Verification the Quick list save functionality for ProblemList
* @Specification 	ProblemList - Save to QUICK LIST - Fields to be saved - ProblemList
...
public class CreateProblemListForQuickList extends AbstractChartPreVisit {
	...
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void createProblemListInAssessMent(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib proData = new ChartPreVisitLib();
		proData.workSheetName = "CreateProblemList";
		proData.testCaseId = "TC_QL_002";
		proData.fetchChartPreVisitTestData();
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		quickListCheckInAssessMent(seleniumHost, seleniumPort, browser, webSite, userAccount, proData);
	}
	...
	***************************************************************************************************/
	public boolean quickListCheck(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib proData) throws IOException{
		...
		try{
			...
			/*assertTrue(click(selenium,"css=button.quic`kListButton"),"Could not click the QuickList button");
			waitForPageLoad(selenium);

			while((Integer) selenium.getXpathCount("//body/div[4]/div/div/div/div/div")>0){
				//a[contains(text(),'Delete')]
				assertTrue(click(selenium,"link=Delete"),"Could not delete the QuickList value;More details:"+proData.toString());
				assertTrue(click(selenium,"css=button.quickListButton"),"Could not click the QuickList button;More details:"+proData.toString());
				assertTrue(click(selenium,"css=button.quickListButton"),"Could not click the QuickList button;More details:"+proData.toString());
				waitForPageLoad(selenium);
			}*/
	...
	***************************************************************************************************/
	...
	public boolean quickListCheckInAssessMent(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib proData) throws IOException{
		Selenium selenium=null;
		boolean returnValue=true;
		try{
			//--------------------------------------------------------------------//
			...
			while(isElementPresent(selenium,ajxAssessmentSearch)){
				...
				selectValueFromAjaxList(selenium,ajxAssessmentSearch,proData.quickList);
				waitForPageLoad(selenium);
			...
			//  Step-5:  Verify Quick List View   //
			//--------------------------------------------------------------------//
			assertTrue(verifyQuickListViewForProblemListInAssessment(selenium,proData,userAccount),"Verify QuickListView Failed",selenium,ClassName,MethodName);
