...
public class CreateStaff extends AbstractSchedulingTest{
	@Test(groups = {"SmokeSingle","SingleThread","Approved", "firefox", "iexplore", "safari", "default" }, description = "Test For Create staff With Interal")
	...
	***************************************************************************************************/
	public boolean addStaff(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib pDGData) throws Exception{
		...
		try{
			...
			System.out.println(getText(selenium,"css=div.scheduling-information-box"));
			if(getText(selenium,"css=div.scheduling-information-box").contains(pDAData.groupName)){
				returnValue = true;
			}
			...
			//  Step-8: Create Staff			  //
			//--------------------------------------------------------------------//
			if(!isElementPresent(selenium,btnAddStaff1))
			{
				assertTrue(click(selenium,btnNextInGroup),"Unable to click the new button",selenium,ClassName,MethodName);
				waitForPageLoad(selenium);	
			}
