...
public class VerifySearchOptionsInDIChart extends AbstractChartPreVisit{
	...
	@Test(groups = {"SingleThread","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Search Option Serice Date ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySearchOptionService(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib diData = new ChartPreVisitLib();
		diData.workSheetName = "CreateDiagnosticReport";
	...
	public boolean verifySearchOption(Selenium selenium, ChartPreVisitLib diData,String userAccount) throws IOException{
		...
		assertTrue(type(selenium,txtMedsSearchTextBox, diData.labTestName),"Could not type test  name", selenium, ClassName, MethodName);
		waitForPageLoad(selenium);
		}else if (diData.testCaseId.equals("TC_CDR_004")){
			assertTrue(type(selenium,txtMedsSearchTextBox, diData.facilityName),"Could not type sending facility name ", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
		}else {
			assertTrue(type(selenium,txtMedsSearchTextBox,date),"Could not type service data", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
		}
		assertTrue(keyPress(selenium,searchTextBox, "\\13"),"Could not perform keypress",selenium, ClassName, MethodName);
		waitForPageLoad(selenium);	

		assertTrue(getText(selenium,vfyUnsignedDiReportsChart).toLowerCase(new java.util.Locale("en","US")).trim().contains(diData.facilityName.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify Search Option failed", selenium, ClassName,MethodName);
		waitForPageLoad(selenium);
		
		assertTrue(getText(selenium,vfyUnsignedDiReportsChart).toLowerCase(new java.util.Locale("en","US")).trim().contains(date.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify Search Option failed", selenium, ClassName,MethodName);
		waitForPageLoad(selenium);
		return true;
