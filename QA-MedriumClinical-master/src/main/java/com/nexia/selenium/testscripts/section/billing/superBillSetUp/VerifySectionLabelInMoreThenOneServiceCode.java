...
public class VerifySectionLabelInMoreThenOneServiceCode extends AbstractBilling{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Adding Super Bill with more than one service code")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void SectionLabelInMoreThanOneServiceCode(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		BillingLib superBillData = new BillingLib();
		superBillData.workSheetName = "SuperBillSetUp";
		superBillData.testCaseId = "TC_SBS_011";
		superBillData.fetchBillingTestData();
		verifySectionLabel(seleniumHost, seleniumPort, browser, webSite, userAccount, superBillData);
	}
	/*************************************************************************************************** 
	* @purpose        To Verify Section Label In More Then One ServiceCode
	* @action 		  verify Section Label In More Then One ServiceCode
	* @expected       To Ensure that the Section Label In More Then One ServiceCode works fine 
	...
	***************************************************************************************************/
	...
	public boolean verifySectionLabel(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, BillingLib superBillData) throws IOException{
		...
		try {
			...
			//--------------------------------------------------------------------//
			//  Step-8: verify section label details //
			//--------------------------------------------------------------------//
			if(!getValue(selenium,txtSectionLabel1).equalsIgnoreCase(superBillData.sectionLabel)){
				Assert.fail("Section label name is not getting displayed in more then one service code");
			}else
				returnValue = true;
