...
public class VerifySaveDetailsInEditStatement extends  AbstractBilling{
	...
	public boolean verifyCancelStatement(Selenium selenium, String amountDue) throws IOException{
		try{
			...
			waitForPageLoad(selenium);
			assertTrue(click(selenium,btnStatementSave),"Could not click the cancel button", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			return true;
		}catch(Exception e){
			e.printStackTrace();
