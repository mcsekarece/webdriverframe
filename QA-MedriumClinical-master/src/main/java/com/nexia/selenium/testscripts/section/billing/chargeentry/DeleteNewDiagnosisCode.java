...
public class DeleteNewDiagnosisCode extends AbstractBilling {
	...
	@Parameters({ "seleniumHost", "seleniumPort", "browser", "webSite",
			"userAccount" })
	public void verifyDeleteDiagnosisCodeCheckDelete(String seleniumHost,
		...
		chargeEntryData.testCaseId = "TC_CE_005";
		chargeEntryData.fetchBillingTestData();
		verifyDeleteDiagnosisCode(seleniumHost, seleniumPort, browser, webSite,
				userAccount, chargeEntryData);
	}


	public boolean verifyDeleteDiagnosisCode(String seleniumHost, int seleniumPort,
		...
		try {
			...
			goToPostNewCharge(selenium, chargeEntryData);
				...
				if (isElementPresent(selenium, btnDelDiagnosisCode)) {
				assertTrue(click(selenium, btnDelDiagnosisCode),
						"Switch Role Failed", selenium, ClassName, MethodName);
				}
				assertTrue(!isElementPresent(selenium, btnDelDiagnosisCode),
						"Delete Button Present, Row not deleted", selenium, ClassName, MethodName);
				assertTrue(!isElementPresent(selenium, lstDiagnosis41),
							"Second Row present, Row not deleted", selenium, ClassName,
							MethodName);
