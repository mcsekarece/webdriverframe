...
public class VerifyDOBEmptyValidation extends AbstractHomeTest{
	...
	public void verifyDOBEmptyValidation(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		HomeLibUnitTest mandatoryData = new HomeLibUnitTest();
		mandatoryData.workSheetName="UnitTest_FV_CM_Field";
		mandatoryData.testCaseId="TC_CMF_003";
		mandatoryData.fetchHomeTestData();
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		verifyDOBMandatoryField(seleniumHost, seleniumPort, browser, webSite, userAccount, mandatoryData);
	}
	 
	/*************************************************************************************************** 
	* @purpose        To verify validation in DOB field with empty
	* @action 		  verifying validation in DOB field with empty
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	* @since  	      Dec 19, 2013	
	***************************************************************************************************/
	public boolean verifyDOBMandatoryField(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLibUnitTest mandatoryData) throws Exception{
		...
		try{
			...
			waitForPageLoad(selenium);
			assertTrue(createMandatoryUserfields(selenium,mandatoryData),"Could not add mandatory field", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			mandatoryData.testCaseId = "TC_FV_CMF_003";
			mandatoryData.fetchHomeTestData();
			waitForPageLoad(selenium);
			...
			waitForPageLoad(selenium);
			assertTrue(click(selenium,lnkSettings),"Could not click Quick Link", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
