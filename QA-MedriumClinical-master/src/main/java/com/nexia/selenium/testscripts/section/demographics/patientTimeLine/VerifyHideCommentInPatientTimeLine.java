...
public class VerifyHideCommentInPatientTimeLine extends AbstractSchedulingTest{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test For Verifying Hide Comment Option  in Patient Time Line ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void hideCommentInPatientTimeLine(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount)throws Exception{
	    ...
	    verifyHideComment(seleniumHost, seleniumPort, browser, webSite, userAccount, patientTimeLineData);
	} 
	
	/*************************************************************************************************** 
	* @purpose        To verify Hide Comment
	* @action 		  verify Hide Comment
	...
	***************************************************************************************************/
	...
	public boolean verifyHideComment(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib patientTimeLineData) throws Exception{
		...
		try{
			...
			if(!selenium.isTextPresent("Hide Comment")&&(selenium.isTextPresent("Edit"))){
				Assert.fail("The link is not present");
			}
			
			assertTrue(click(selenium,lnkHideComment),"Could not click the Hide Comment", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			if(isElementPresent(selenium,imgCommentTextPopUp)){
				returnValue = true;
			}else
				Assert.fail("The Text Pop up is not present");
