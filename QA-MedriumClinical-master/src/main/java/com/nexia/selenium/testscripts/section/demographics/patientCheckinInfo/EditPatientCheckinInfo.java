...
public class EditPatientCheckinInfo extends AbstractHomeTest{
	...
	public void editPatientInfo(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		editPatientInfo(seleniumHost, seleniumPort, browser, webSite, userAccount, patientData);
	}
	@Test(groups = {"RegressionUS","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Editing patient Info") 
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void editPatientInfoUsingAdvanceSearch(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		editPatientInfoData.workSheetName = "EditPatientInfo";
	...
	/*************************************************************************************************** 
	* @purpose        To verify edit Patient Info
	* @action 		  verifying edit Patient Info
	* @author         Aspire QA
	 * @throws IOException 
	* @state          Developing
	* @useraccount    Both
	* @since  	      Feb 06, 2012
	***************************************************************************************************/
	public boolean editPatientInfo(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLib patientData) throws InterruptedException, IOException{
		...
		try{
			...
			EditPatientData.workSheetName = "EditPatientInfo";
			...
			if(!EditPatientData.responsibleProvider.isEmpty())
				if(!EditPatientData.responsibleProvider.contains("N/A"))
					if(!selectValueFromAjaxList(selenium,"responsibleProviderSuggestBoxIdsuggestBox",EditPatientData.responsibleProvider))
						Assert.fail("Could not Enter responsible Provider, Expected value to be selected" + EditPatientData.responsibleProvider);
			if(!EditPatientData.pcpCheck.isEmpty())
				if(!EditPatientData.pcpCheck.contains("No"))
					assertTrue(check(selenium, chkResponsibleProvider), "Could not check the pcp check box, Expected value for the check box : " + EditPatientData.pcpCheck, selenium, ClassName, MethodName);
			...
			if(!EditPatientData.maritalStatus.isEmpty())
				if(!EditPatientData.maritalStatus.contains("N/A"))
					if(!selectMatchingValueFromAjaxList(selenium, ajxmartialStatus, EditPatientData.maritalStatus))
			...
			if(!EditPatientData.contactPreferences.isEmpty())
				if(!EditPatientData.contactPreferences.contains("N/A"))
					if(!selectMatchingValueFromAjaxList(selenium,"//div[12]/div/div/div/div/div/div/input",EditPatientData.contactPreferences))
				...
				if(!EditPatientData.sameAsResidentialAddressCheck.contains("yes"))
					...
					if(!EditPatientData.mailingAddressCity.isEmpty())
						...
						if(!EditPatientData.mailingAddressState.contains("N/A"))
							if(!selectMatchingValueFromAjaxList(selenium, ajxMailingState, EditPatientData.mailingAddressState))
			...
			if(!verifyStoredValuesEditPatient(selenium,EditPatientData)){
	...
	public boolean editPatientInfoAdvanceSearch(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLib editPatientInfoData) throws InterruptedException, IOException{
		...
		try{
			...
			searchPatientNexia(selenium,editPatientInfoData.patientID);
			waitForPageLoad(selenium);
			assertTrue(click(selenium,lnkPatientInfoView),"Could not click the patient info link;More details:"+editPatientInfoData.toString(), selenium, ClassName, MethodName);
			click(selenium,btnRegistrationEdit);
			waitForPageLoad(selenium);
			...
			if(!editPatientInfoData.responsibleProvider.isEmpty())
				...
				if(!selectValueFromAjaxList(selenium,"responsibleProviderSuggestBoxIdsuggestBox",editPatientInfoData.responsibleProvider))
			...
			if(!editPatientInfoData.sex.isEmpty())
				if(!editPatientInfoData.sex.contains("N/A"))
					if(!selectMatchingValueFromAjaxList(selenium, ajxSex, editPatientInfoData.sex))
			...
			if(!editPatientInfoData.contactPreferences.isEmpty())
				if(!editPatientInfoData.contactPreferences.contains("N/A"))
					assertTrue(type(selenium, "//div[12]/div/div/div/div/div/div/input", editPatientInfoData.contactPreferences), "Could not Enter email, Expected value to be Typed" + editPatientInfoData.email, selenium, ClassName, MethodName);
			
			if(!editPatientInfoData.sameAsResidentialAddressCheck.isEmpty()){
				if(!editPatientInfoData.sameAsResidentialAddressCheck.contains("No"))
					assertTrue(click(selenium, chkMailingAsResidential), "Could not check the ssn check box, Expected value for the check box : " + editPatientInfoData.sameAsResidentialAddressCheck, selenium, ClassName, MethodName);
				else{
					if(isChecked(selenium,chkMailingAsResidential))
						assertTrue(click(selenium, chkMailingAsResidential), "Could not check the ssn check box, Expected value for the check box : " + editPatientInfoData.sameAsResidentialAddressCheck, selenium, ClassName, MethodName);
					...
					if(!editPatientInfoData.mailingAddressCity.isEmpty())
						...
						if(!editPatientInfoData.mailingAddressState.contains("N/A"))
							if(!selectMatchingValueFromAjaxList(selenium, ajxMailingState, editPatientInfoData.mailingAddressState))
			...
			if(!verifyStoredValuesEditPatient(selenium,editPatientInfoData)){
