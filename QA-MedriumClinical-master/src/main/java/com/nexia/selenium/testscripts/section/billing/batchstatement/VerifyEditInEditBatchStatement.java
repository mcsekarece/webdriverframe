...
public class VerifyEditInEditBatchStatement extends  AbstractBilling {
	...
	public void verifyEditDetails(String seleniumHost, int seleniumPort, String browser, String webSite, String userAccount) throws Exception {
			...
			verifyEditDetails(seleniumHost, seleniumPort, browser, webSite, userAccount, billingData);
	    }
	 
	 /**
	     *  To Verify Edit Details in Edit Statement
	     * @param seleniumHost
	     * @param seleniumPort
	     * @param browser
	     * @param webSite
	 ...
	 public boolean verifyEditDetails(String seleniumHost, int seleniumPort, String browser, String webSite, String userAccount, BillingLib billingData)throws Exception {
			...
			// --------------------------------------------------------------------//
			// Step-7:verifyEditDetails
			// --------------------------------------------------------------------//
				
				String ProName[]=chargeEntryData.renderingProvider.split(",");				
				String Providername="Dr."+ProName[1].toUpperCase()+" "+ProName[0].toUpperCase();
                  System.out.println(Providername);	
			if(!getText(selenium,EditStatemetDOS).contains(chargeEntryData.dateOfService1))
			{
				return false;
			}
			if(!getText(selenium,EditStatemetserCode).contains(chargeEntryData.serviceCode))
			{
				return false;	
			}
			if(!getText(selenium,EditStatementProvoder).contains(Providername))
			{
				return false;	
			}
			if(!getText(selenium,EditStatementPayerCode).contains(payerData.payerCode))
			{
				return false;	
			}
			if(!selenium.isChecked(EditStatementcheckbox))
			{
				return false;	
			}
