...
public class VerifyTakePlaceValidationInSessionNote  extends AbstractSchedulingTest{
	@Test(groups = {"SingleThread","Approved", "firefox", "iexplore", "safari", "default" }, description = "Test For  Verify TakePlace Validation In SessionNote")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void TakePlaceVanlidationInSessionNote(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		VerifyTakePlaceValidation(seleniumHost, seleniumPort, browser, webSite, userAccount, pDGData);
	}
	
	/*************************************************************************************************** 
	* @purpose        To Verify Take Place Validation
	* @action 		   Verify Take Place Validation
	* @expected       To Ensure that the  Verify Take Place Validation works fine 
	...
	* @since  	      Dec 18, 2013
	***************************************************************************************************/
	public boolean VerifyTakePlaceValidation(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib pDGData) throws Exception{
		...
		try{
			...
			assertTrue(getValue(selenium,txtTimeMins).contains(mins),"Mins is not getting displayed properly" ,selenium, ClassName, MethodName);
			//--------------------------------------------------------------------//
			//  Step-10:Verify Take Palce Validation //
			//--------------------------------------------------------------------//
			selectValueFromajxListForUnit(selenium,"locationSuggestBoxsuggestBox","");
			waitForPageLoad(selenium);
			...
			if(!(isElementPresent(selenium,"locationSuggestBoxsuggestBox")&&(selenium.isTextPresent("Location cannot be empty")))){
				Assert.fail("Alert is not getting displayed");
			}else
				returnValue = true;
