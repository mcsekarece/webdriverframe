...
public class VerifyMessageLeftInWaitListQueue extends AbstractOperationsTest{

	@Test(groups = {"Regression","Review","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Wait List message left in wait list queue")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyMessageLeftInWaitListQueue(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	/*************************************************************************************************** 
	* @purpose        verify Wait List message left in wait list queue
	* @action 		  verifying Wait List message left in wait list queue
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	***************************************************************************************************/
	...
	public boolean waitListManagement(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, OperationsTestLib waitListDataObj1 ) throws IOException{
		...
		try{
			...
			//---------------------------------------------------------------------//
			//  Step-12: Click 'left a message' in Wait list 		               //
			//---------------------------------------------------------------------//
			assertTrue(click(selenium, btnLeftMessage), "Could not click on left a message", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			//---------------------------------------------------------------------//
			//  Step-14: Verify 'left a message' in Wait list queue	               //
			//---------------------------------------------------------------------//
			if(!getText(selenium,lnkFirstWaitListQueue).toLowerCase(new java.util.Locale("en","US")).trim().contains("Message left".trim().toLowerCase(new java.util.Locale("en","US")))){
				 return false;                       
			}
