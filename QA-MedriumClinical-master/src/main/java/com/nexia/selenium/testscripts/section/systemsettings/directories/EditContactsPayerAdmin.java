...
public class EditContactsPayerAdmin  extends AbstractSystemSettingsTest {
	
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Editing new Contact for New Payer Admin")
	...
	*/
	...
	public boolean createPayerAdmin(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SystemSettingsLib payerAdminData) throws IOException{
		...
		try{
			...
			assertTrue(checkPayerPicklist(selenium),"Pick list will show", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			//--------------------------------------------------------------------//
			//  Step-7: Edit the previously created Contact			  			 //
			//--------------------------------------------------------------------//
			...
			assertTrue(click(selenium,btnEditContact),"Could not click on edit button Contact; More details"+payerAdminData.toString(), selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			//--------------------------------------------------------------------//
			//  Step-8: Verify the previously created edited Contact			  					  //
			//--------------------------------------------------------------------//
			assertTrue(verifyContactValuesInSummaryPage(selenium,payerAdminData, userAccount), 
					"Payer Contact details not saved proberly:"+payerAdminData.toString(), selenium, ClassName, MethodName);
