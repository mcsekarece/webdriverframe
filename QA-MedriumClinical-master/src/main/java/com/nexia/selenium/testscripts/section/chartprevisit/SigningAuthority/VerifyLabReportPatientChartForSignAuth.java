...
public class VerifyLabReportPatientChartForSignAuth extends AbstractChartPreVisit {	
	...
	public void verifyFilingDocumentsWithFullAccessSA(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		accessData.testCaseId = "TC_SA_034";
	...
	public void verifyFilingDocumentsWithLimitedAccessSA(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib accessData = new ChartPreVisitLib();
		accessData.workSheetName = "SigningAuthority";
		accessData.testCaseId = "TC_SA_035";
		accessData.fetchChartPreVisitTestData();
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		verifyFileReportWithSigningAuthority(seleniumHost, seleniumPort, browser, webSite, userAccount, accessData);
	...
	public boolean verifyFileReportWithSigningAuthority(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib accessData) throws Exception{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-5: Navigate to File Lab Report page in Patient chart
			//--------------------------------------------------------------------//
			assertTrue(searchPatientNexiaForProviderHomePage(selenium, accessData.patientId), 
					"Could not find patient with Id " + accessData.patientId, selenium, ClassName, MethodName);
			
			assertTrue(navigateTofileReport(selenium,accessData), "Could not navigate to File Report page", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);

			assertTrue(selectValueFromAjaxList(selenium, ajxFileReportType, "Lab Reports"),"Could not select Lab.", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium, btnEnterData),"Could not click Enter Data >> button.", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);			
			...
			assertTrue(selenium.isTextPresent("File Report"),"Not able to file " +accessData.documentType, selenium, ClassName, MethodName);
	...
	private boolean verifySignedLabReport(Selenium selenium,	ChartPreVisitLib accessData) throws IOException {
		...
		if(isElementPresent(selenium, lnkTabLabs)) {
			//Select Labs Tab		
			assertTrue(click(selenium, lnkTabLabs),"Could not click the link Lab Tests", selenium, ClassName, MethodName);
		}else{
