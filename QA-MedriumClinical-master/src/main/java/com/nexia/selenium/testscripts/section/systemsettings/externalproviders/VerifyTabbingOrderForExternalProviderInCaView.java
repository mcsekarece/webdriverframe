...
public class VerifyTabbingOrderForExternalProviderInCaView extends AbstractSystemSettingsTest{
	@Test(groups = {"RegressionCA","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Adding a External Provider")
	...
	public void verifyTabingOrderForAdminExternalProvider(String seleniumHost,int seleniumPort, String browser, String webSite, String userAccount,SystemSettingsLib providerTabOrder) throws IOException{
		...
		String csspaths[]={"css=#referringCode","css=#lastName","css=#mi","css=#credentials","css=#stateLicNum","css=#suggestBox","css=#billingExclusioncheckbox","css=#pecoscheckbox","css=#billingNumber","css=#comments","xpath=//input[starts-with(@id, 'practiceName')]","xpath=(//input[@type='text'])[16]"};
		try{
			//--------------------------------------------------------------------//
			...
			Assert.assertNotNull(selenium,"Could Not retrieve the New Selenium Session; More Details :" + providerTabOrder .toString());
			...
			//--------------------------------------------------------------------//
			assertTrue(switchRole(selenium,providerTabOrder.switchRole),"Swicth role failed",selenium, ClassName, MethodName);
			...
			for(int i =0;i<csspaths.length;i++){
				...
				assertTrue(selenium.getElementIndex("dom=document.activeElement") == selenium.getElementIndex(csspaths[i]), 
						"Index not matched", selenium, ClassName, MethodName);
