...
public class VerifyChartsNotesHidden extends AbstractHomeTest {
	...
	public void verifyChartsNotesVisibleOnDemo(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		PatientData.testCaseId = "TC_NC_011";
	...
	***************************************************************************************************/
	public boolean verifyChartsNotesVisible(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLib PatientData) throws Exception{
		...
		try{
			...
			waitForPageLoad(selenium);
			//Patient Notes is hidden
			
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in patient summary section ", selenium, ClassName, MethodName);
			
			click(selenium,lnkSocioEconomics);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in socio economic section ", selenium, ClassName, MethodName);
			click(selenium,lnkHouseHold);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in House Hold section ", selenium, ClassName, MethodName);
			click(selenium,lnkContactList);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in House Hold section ", selenium, ClassName, MethodName);
			click(selenium,lnkIdentifier);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in Identifiers section ", selenium, ClassName, MethodName);
			click(selenium,lnkReferrals);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in Referrals section ", selenium, ClassName, MethodName);
			click(selenium,lnkConsentsPatient);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in Consents section ", selenium, ClassName, MethodName);
			click(selenium,lnkNotes);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in Notes section ", selenium, ClassName, MethodName);
			click(selenium,lnkPatientCaseList);
			waitForPageLoad(selenium);
			assertTrue(verifyNotesHidden(selenium, PatientData),"Charts notes not visible in Cases section ", selenium, ClassName, MethodName);
	...
	public boolean verifyNotesHidden(Selenium selenium, HomeLib PatientData){
		if(isElementPresent(selenium,lblPatientNoteShow)){
			return true;
		}
		else return false;
	}
