...
public class VerifyCancelInAttendanceGroupParticipants extends AbstractSchedulingTest{
	
	@Test(groups = {"SingleThread","Approved", "firefox", "iexplore", "safari", "default" }, description = "Test For verify cancel in Attendance Group Participants")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyCancelInAttendanceGroupParticipants(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	/*************************************************************************************************** 
	* @purpose        To verify cancel in Attendance Group Participants
	* @action 		  verify cancel in Attendance Group Participants
	* @expected       To Ensure that the cancel functionality working in Attendance Group Participants
	...
	***************************************************************************************************/
	public boolean createProgramGroup(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib pdgSeriesData) throws Exception{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-9: verify cancel in attendance	group participants			  //
			//--------------------------------------------------------------------//
			...
			assertTrue(click(selenium,btnCancelAttendanceGroup),"Could not click the cancel in attendance group", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			if(!isElementPresent(selenium, btnCancelRecordAttendance)){
			       return false;
			}
		}
