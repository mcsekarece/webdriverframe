...
public class VerifyCancelOptionForLabReport extends AbstractChartPreVisit {
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify the Canacle option for Lab Report")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void cancelLabReport(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		ChartPreVisitLib labData = new ChartPreVisitLib();
		labData.workSheetName = "FileLabReport";
		labData.testCaseId = "TC_LR_007";
		labData.fetchChartPreVisitTestData();
		cancelReport(seleniumHost, seleniumPort, browser, webSite, userAccount, labData);
	}
	/**
	 * cancelReport
	 ...
	 * @since  	    Dec 07, 2012
	 */
	public boolean cancelReport(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib labData) throws IOException{
		...
		try{
			...
			if(isElementPresent(selenium,lnkActionsLink)){
					returnValue = false;
				}else if(isElementPresent(selenium,btnSign)){
