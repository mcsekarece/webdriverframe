...
public class VerifyCountInOrderNotofocation extends AbstractChartPreVisit{
	...
	public void VerifyNotifyMeBlockInLab(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		orderdata.testCaseId = "TC_ON_004";
	...
	public boolean VerifyNotifyMeBlock(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib orderdata) throws Exception{
		...
		try
		{
			...
			//--------------------------------------------------------------------//
			//  Step-2: Get the Count of Order Notification Queue Before add lab order 
			//--------------------------------------------------------------------//
			String BeforeCountStr=selenium.getText(lblCountOrder);
			int Beforecount =Integer.parseInt(BeforeCountStr);
			//--------------------------------------------------------------------//
			//  Step-3:Search Patient in Application 
			//--------------------------------------------------------------------//
			...
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
			//  Step-7:Add lab report with Notify 
			//--------------------------------------------------------------------//
			ChartPreVisitLib orderData = new ChartPreVisitLib();
			...
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
			//  Step-9:Get After Count and Compare It With Before count 
			//--------------------------------------------------------------------//
			/*String AfterCountStr=selenium.getText(lblCountOrder);
			int Aftercount =Integer.parseInt(AfterCountStr);
			...
			{
				Assert.fail("Order notification count is not updated ");
			}*/
		}
		catch (Exception e)
		{	
