...
public class VerifyOnbehalfOfForLab extends AbstractChartPreVisit{
...
try
{
		...
		if(encounterData.testCaseId.equals("TC_CO_007"))
		{
			...
			assertTrue(clickLabIcon(selenium,encounterData),"Navigation Failed", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
			//  Step-5:Verify default value//
			//--------------------------------------------------------------------//
	assertTrue(verifySuggestBox(selenium,encounterData),"Verifying On behalf of suggest box in Create lab page failed (suggest box mighnt not have supervisor or current provider name)", selenium, ClassName, MethodName);
...
catch(Exception e)
{
	...
	public boolean searchCurrentProviders (Selenium selenium,String ProviderName) throws IOException{
		...
		assertTrue(click(selenium,btnActiveHealth),"Can not Click Active Health", selenium, ClassName, MethodName);
			}	
