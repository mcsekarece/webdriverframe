...
public class CreateWidgetsInVisitSection extends AbstractClinicalSettings{

	@Test(groups = {"AdvancedSmoke", "Approved","Regression","firefox", "iexplore", "safari", "default" }, description = "Test for creating Encounter Structure template EndToEnd")
...
public boolean encounterTemplate(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalSettingLib encounterTemplateTestData) throws IOException{
		...
		//	assertTrue(click(selenium, btnComponentEdit), "Could not click on Edit component", selenium, ClassName, MethodName);
			...
			//--------------------------------------------------------------------//		
			assertTrue( searchPatientNexiaForProviderHomePage(selenium,PatientData.lastName),"Could not search patient", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
			...
			waitForPageLoad(selenium);
			if(isElementPresent(selenium,continueButton))
			{
				click(selenium,continueButton);
			}
			//--------------------------------------------------------------------//
			...
			assertTrue(clickAt(selenium,btnSign,""),"Could not click the Sign button in encounter note ", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
