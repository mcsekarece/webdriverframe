...
public class VerifyConfirmVisitInPatientTimeLine extends AbstractSchedulingTest{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test For Verifying Confirm Visit Details in Patient Time Line ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void confirmVisit(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount)throws Exception{
	    ...
	    verifyConfirmVisit(seleniumHost, seleniumPort, browser, webSite, userAccount, patientTimeLineData);
	} 
	
	/*************************************************************************************************** 
	* @purpose        To verify Confirm Visit Status 
	* @action 		  verify Confirm Visit Status
	* @expected       To Ensure that the verify Confirm Status works fine    
	* @author         Aspire Qa
	* @state          Developing
	...
	***************************************************************************************************/
	...
	public boolean verifyConfirmVisit(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib patientTimeLineData) throws Exception{
		...
		try{
			...
			assertTrue(click(selenium,lnkConfirmVisit),"Could not click the Confirm visit", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			//--------------------------------------------------------------------//
			//  Step-11 :Verify  Confirm Visit details //
			//--------------------------------------------------------------------//
			selectValueFromAjaxList(selenium,ajxshowFillter,"Confirmed");
			waitForPageLoad(selenium);
	...
	public boolean verifyConfirmVistDetails(Selenium selenium,SchedulingTestLib patientTimeLineData , String providerName, String location) throws IOException{
		...
		assertTrue(getText(selenium,lblExpandPatientTimeLine).contains("Scheduled"),"could not get text",selenium,ClassName,MethodName);
		return true;
	}
}
