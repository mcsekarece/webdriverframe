...
public class VerifySearchOptionIconDIChart extends AbstractChartPreVisit{
	
	@Test(groups = {"SingleThread","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Search Option XMark Icon in chart")
	...
	/**
	 * Verify Search Option XMark Icon in chart
	 * function to Verify Search Option XMark Icon in chart
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	...
	try{
			...
			// Verify the default search options in search fields
			
			assertTrue(selenium.getAttribute(searchTextBox).trim().equalsIgnoreCase("Search by test name,service date or sending"),"Verify search field hint failed", selenium, ClassName,MethodName);  
            waitForPageLoad(selenium);
			
			// -------------------------------------------------------------------//
			//  Step-7 : Verify the report created 				      		      // 
			//--------------------------------------------------------------------//
			...
			assertTrue(getText(selenium,vfyUnsignedDiReportsChart).toLowerCase(new java.util.Locale("en","US")).trim().contains(diData1.facilityName.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify DI  report failed", selenium, ClassName,MethodName);
		    waitForPageLoad(selenium);
		    	
		  // -------------------------------------------------------------------//
		 //  Step-8 : Verify the search icon		      		               // 
		//--------------------------------------------------------------------//
		   
		    assertTrue(click(selenium,SearchCloseicon),"Could not click Search Close icon ", selenium, ClassName,MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(!getText(selenium,vfyUnsignedDiReportsChart).toLowerCase(new java.util.Locale("en","US")).trim().contains(diData1.facilityName.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify search icon failed", selenium, ClassName,MethodName);
		    waitForPageLoad(selenium);
