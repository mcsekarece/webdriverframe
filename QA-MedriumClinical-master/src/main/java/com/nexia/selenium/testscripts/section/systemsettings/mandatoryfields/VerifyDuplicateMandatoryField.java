...
public class VerifyDuplicateMandatoryField extends AbstractSystemSettingsTest {
	...
	public void verifyDuplicateMandField(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		verifyDuplicateMandatoryField(seleniumHost, seleniumPort, browser, webSite, userAccount, mandData);
	}
	
	/**
	 * verify Duplicate Mandatory Field
	 * function to verify Duplicate Mandatory Field
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	...
	public boolean verifyDuplicateMandatoryField(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SystemSettingsLib mandData) throws Exception{
		...
		try{
			...
			if(!isElementPresent(selenium,lblPatientIdentifierGroup)){
				if(isElementPresent(selenium,"errorCloseButton")){
					waitForPageLoad(selenium);
					if(getText(selenium,"css=li > div").equals("css=li > div")){
						 returnValue=true;
					}else 
						returnValue=false;
					}
				...
				Assert.fail("Able to create duplicate mandatory field:" + mandData.toString());
