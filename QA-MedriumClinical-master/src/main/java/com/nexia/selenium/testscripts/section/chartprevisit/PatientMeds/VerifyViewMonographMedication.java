...
public class VerifyViewMonographMedication extends AbstractChartPreVisit{
	...
	public void verifyViewMonograph(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	***************************************************************************************************/
	...
	public boolean verifyMonogarphMedication(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib prescribeData)throws Exception{
		...
		try{
			...
			selectValueFromAjaxList(selenium,ajxMedShow,"C");
			waitForPageLoad(selenium);
			...
			if(getText(selenium,lblCureentMedsname).toLowerCase(new java.util.Locale("en","US")).trim().contains(prescribeData.prescribeCa.trim().toLowerCase(new java.util.Locale("en","US")))){
				...
				assertTrue(click(selenium,"//a[contains(@id,'openmonograph')]"),"Could not click View Monograph button", selenium, ClassName, MethodName);
				waitForPageLoad(selenium);
	...
	public boolean verifyStoreValueForMonograph(Selenium selenium,ChartPreVisitLib prescribeData) throws IOException{
		...
		assertTrue(click(selenium,"section_0"),"Could not click the Dosage link", selenium, ClassName, MethodName);
		if(!selenium.isTextPresent("Dosage")){
			return false;
		}
		
		assertTrue(click(selenium,"section_1"),"Could not click the Dosage link", selenium, ClassName, MethodName);
		if(!selenium.isTextPresent("Indications")){
			return false;
		}
		
		if(!isElementPresent(selenium,"xpath=(//a[contains(text(),'More')])[2]")){
			return false;
		}
		return true;
