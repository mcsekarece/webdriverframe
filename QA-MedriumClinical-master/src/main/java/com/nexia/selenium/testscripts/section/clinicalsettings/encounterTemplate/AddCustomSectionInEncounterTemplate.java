...
public class AddCustomSectionInEncounterTemplate extends AbstractClinicalSettings {
	
	@Test(groups = {"SingleThread","Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verifing add custom sections in encounter template")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void addCustomSectionAndComponentInEncounterTemplate(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	/*************************************************************************************************** 
	* @purpose        To verify add custom sections in encounter template
	* @Specification  SRS_EMR_Encounter_Structure_Templates Rev0.0
	* @action 		  Verifying add custom sections in encounter template
	* @author         AspireQA
	* @throws 		  IOException 
	* @state          Developing
	* @useraccount    Both
	...
	***************************************************************************************************/
	...
	public boolean encounterTemplate(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalSettingLib encounterTemplateTestData) throws IOException{
		...
		try{
			...
			{
				assertTrue(click(selenium, lblEncountertemplate2), "Could not click search result", selenium, ClassName, MethodName);
				waitForPageLoad(selenium);
			...
			//--------------------------------------------------------------------//
			assertTrue(VerifyCustomSectionAndComponent(selenium, encounterTemplateTestData),"Verification failed for added encounter template in summary page", selenium, ClassName, MethodName);
