...
public class VerifyCustomMandatoryFieldWithRace extends AbstractHomeTest {
	...
	public void createMandatoryField(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		mandatoryData.testCaseId="TC_CMF_002";
	...
	/**
	 * create Custom Mandatory Field
	 * function to create Custom MandatoryField
	/*************************************************************************************************** 
	...
	***************************************************************************************************/
	public boolean createCustomMandatoryField(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLibUnitTest mandatoryData) throws Exception{
		...
		try{
			...
			mandatoryData.testCaseId = "TC_FV_CMF_002";
			mandatoryData.fetchHomeTestData();
			...
			selectValueFromAjaxList(selenium, ajxStatus, mandatoryData.status);
			selectValueFromAjaxList(selenium,ajxRace,mandatoryData.livingArrangement);
			waitForPageLoad(selenium);
			assertTrue(type(selenium,txtRaceOther,mandatoryData.raceOther),"Could not type other Value for race;More Details:"+mandatoryData.toString(), selenium, ClassName, MethodName);
			...
			//--------------------------------------------------------------------//
			//  Step-8: Go to System Settings and navigate to Mandatory User Field//
			//--------------------------------------------------------------------//
			click(selenium,btnNexiaLogo);
			waitForPageLoad(selenium);
			assertTrue(click(selenium,lnkQuickLink),"Could not click Quick Link", selenium, ClassName, MethodName);
			...
			assertTrue(isElementPresent(selenium,lnkSystemSetting),"Could not frind the link", selenium, ClassName, MethodName);
			assertTrue(click(selenium,lnkSystemSetting),"Could not frind the link", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			assertTrue(click(selenium,lnkMandatoryUserFields),"Could not click on Health Care Team link:" + mandatoryData.toString(), selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			//---------------------------------------------------------- ----------//
			//  Step-9: Deleting existing Mandatory User Field //
			//--------------------------------------------------------------------//
