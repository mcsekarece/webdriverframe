...
public class VerifyServiceDateAscendingOrderinDIChart extends AbstractChartPreVisit{
	
	@Test(groups = {"SingleThread","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for verify Service Date Ascending Order in DI chart")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyServiceDateAscendingOrderInChart(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	public boolean createDiagnosticTestReport(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib diData) throws IOException{
		...
		try{
			...
			assertTrue(verifyServiceDateAscendingOrderInDIChart(selenium,diData,diData1,userAccount),"Verify ServiceDate Ascending Order failed", selenium, ClassName,MethodName);
	...
	public boolean verifyServiceDateAscendingOrderInDIChart(Selenium selenium, ChartPreVisitLib diData, ChartPreVisitLib diData1,String userAccount) throws IOException{
	   ...
	   //Verify service Date 
		...
		assertTrue(getText(selenium,vfyUnsignedDiReportsChart).toLowerCase(new java.util.Locale("en","US")).trim().contains(date1.toLowerCase(new java.util.Locale("en","US"))),"Verify ServiceDate  failed", selenium, ClassName,MethodName);
		waitForPageLoad(selenium);
		
		assertTrue(getText(selenium,vfyUnsignedDiReportsChart).toLowerCase(new java.util.Locale("en","US")).trim().contains(date.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify ServiceDate failed", selenium, ClassName,MethodName);
		waitForPageLoad(selenium);
		
		//Verify test name 	

		assertTrue(getText(selenium,vfyUnsignedDiReportsChart1).toLowerCase(new java.util.Locale("en","US")).trim().contains(diData1.labTestName.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify Test Name failed", selenium, ClassName,MethodName);
		waitForPageLoad(selenium);
		
		assertTrue(getText(selenium,vfyUnsignedDiReportsChart1).toLowerCase(new java.util.Locale("en","US")).trim().contains(diData.labTestName.trim().toLowerCase(new java.util.Locale("en","US"))),"Verify Test Name  failed", selenium, ClassName,MethodName);
		waitForPageLoad(selenium);
