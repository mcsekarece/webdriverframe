...
public class VerifySignPendingMedication extends AbstractChartPreVisit{
	@Test(enabled=false)
	...
	public void addPendingMedication(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		prescribeData.testCaseId = "TC_PM_099";
		prescribeData.fetchChartPreVisitTestData();
		
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		
		signPendingMedication(seleniumHost, seleniumPort, browser, webSite, userAccount, prescribeData);
	}
	/**
	 * signPendingMedication
	 * Function to verify sign for Pending medication
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	 * @since  	    Sep 19, 2012
	 */
	public boolean signPendingMedication(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib prescribeData) throws Exception{
		...
		try{
			...
			assertTrue(click(selenium, btnSign), "Could not click on Approve button", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
		
			assertTrue(click(selenium, lnkMedicationTab), "Could mot click on medication tab", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			assertTrue(click(selenium, lnkOption), "Could mot click on option list", selenium, ClassName, MethodName);
			if(!getValue(selenium, lnlPendingDelete1).equalsIgnoreCase("Delete")){
				return returnValue;
			}else
				returnValue=false;
