...
public class EditPDGSeries extends AbstractSchedulingTest{
	@Test(groups = {"SingleThread","Approved", "firefox", "iexplore", "safari", "default" }, description = "Test For Edit PDG series ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void editScheduleSeries(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		editSeries(seleniumHost, seleniumPort, browser, webSite, userAccount, pDGData);
	}
	
	/*************************************************************************************************** 
	* @purpose        To edit PDG Series
	* @action 		  verify edit PDG Series
	* @expected       To Ensure that the edit PDG Series works fine 
	...
	* @since  	      Oct 29, 2013
	***************************************************************************************************/
	public boolean editSeries(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib pDGData) throws Exception{
		...
		try{
			...
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
			//  Step-8: Verify PG Page			  //
			//--------------------------------------------------------------------//
			
			assertTrue(type(selenium,txtSearchProgram,pDAData.groupName),"Could not type the Program name",selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			assertTrue(getText(selenium,lblPSeries).contains(pDGData.seriesName),"Verification failed",selenium, ClassName, MethodName);
			assertTrue(getText(selenium,lblPSeries).contains(startDate),"Verification failed",selenium, ClassName, MethodName);
			

			/*assertTrue(click(selenium,lnkProgramAction),"Could not click the action link",selenium,ClassName,MethodName);
			waitForPageLoad(selenium);
			assertTrue(click(selenium,lnkViewSeries), "could not click the view all series", selenium, ClassName, MethodName);*/
			
			assertTrue(click(selenium, lnkEdit), "could not able to click the edit series", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
