...
public class VerifyHideOptionInFamilyHistoryFromChartSummary extends AbstractChartPreVisit{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Hide Option In Family History From Chart Summary Page")
	...
	public void verifyHideOptionInFamilyHistory(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		customiszeData.workSheetName = "CreateFamilyHistory";
		customiszeData.testCaseId = "TC_CFM_029";
	...
	/*************************************************************************************************** 
	* @purpose        To verify Hide option in Family History From Chart Summary Page
	* @action 		  verifying Hide option in Family History From Chart summary page 
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	public boolean verifyHideOption(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount,ChartPreVisitLib customiszeData) throws IOException{
		...
		try{
			...
			if(isChecked(selenium,chkFamilHistoryOptionalElement))
				assertTrue(click(selenium,chkFamilHistoryOptionalElement),"Could not click the Check box", selenium, ClassName, MethodName);
			...
			assertTrue(deleteFamilyHistory(selenium,customiszeData),"Deletion Failed", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			assertTrue(createFamilyHistory(selenium,customiszeData),"Could not create the Family History", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			//--------------------------------------------------------------------//
			assertTrue(checkOptionalElement(selenium, sectionName1, chkFamilHistoryOptionalElement), "Could not Find the optional Element", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
