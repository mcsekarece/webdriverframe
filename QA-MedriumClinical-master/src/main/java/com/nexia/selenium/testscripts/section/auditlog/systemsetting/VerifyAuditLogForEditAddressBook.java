...
public class VerifyAuditLogForEditAddressBook extends AbstractAuditLog{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Adding a New patient Case")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite"})
	public void editAddressbookForAuditLog(String seleniumHost, int seleniumPort,String browser, String webSite,String userAccount) throws Exception {
		AuditLogLib userData = new AuditLogLib();
		userData.workSheetName = "AuditLogForAddressBook";
		userData.testCaseId = "TC_ALAB_001";
		userData.fetchHomeTestData();
		editAddressbook(seleniumHost, seleniumPort, browser, webSite, userData,userAccount);
	}
	/**
	 * editAddressbook
	 * function to edit Addressbook
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	 ...
	 * @since  	   	Jan 22, 2012
	 */
	public boolean editAddressbook(String seleniumHost, int seleniumPort,String browser, String webSite,AuditLogLib adminAddrData,String userAccount) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//---------------  Step-8:Edit AddressBook  --------------------------// 
			//--------------------------------------------------------------------//
			adminAddrData.workSheetName = "AuditLogForAddressBook";
			adminAddrData.testCaseId = "TC_ALAB_002";
			adminAddrData.fetchHomeTestData();
			
			click(selenium,"//div[2]/div/div/div/span/button");
			waitForPageLoad(selenium);
			
			Assert.assertTrue(createAddressBook(selenium,adminAddrData),"Edit Failed");
			waitForPageLoad(selenium);
