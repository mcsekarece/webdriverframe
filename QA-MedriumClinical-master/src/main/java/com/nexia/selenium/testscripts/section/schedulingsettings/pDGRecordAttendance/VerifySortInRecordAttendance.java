...
public class VerifySortInRecordAttendance extends AbstractSchedulingTest{
	
	@Test(groups = {"SingleThread","Approved", "firefox", "iexplore", "safari", "default" }, description = "Test For verify sort in record attendance")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySortInRecordAttendance(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		SchedulingTestLib pdgSeriesData = new SchedulingTestLib();
	    pdgSeriesData.workSheetName = "ProgramAddSeries";
	    pdgSeriesData.testCaseId = "TC_PAS_007";
	...
	/*************************************************************************************************** 
	* @purpose        To verify sort in record attendance
	* @action 		  verify sort in record attendance
	* @expected       To Ensure that the sort functionality in record attendance
	...
	***************************************************************************************************/
	public boolean createProgramGroup(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SchedulingTestLib pdgSeriesData) throws Exception{
		...
		try{
			...
			//--------------------------------------------------------------------//
			SchedulingTestLib pdgSeriesData1 = new SchedulingTestLib();
		    pdgSeriesData1.workSheetName = "ProgramAddSeries";
		    pdgSeriesData1.testCaseId = "TC_PAS_008";
		    pdgSeriesData1.fetchSchedulingTestData();
		    
			assertTrue(addNewSeries(selenium,pdgSeriesData1),"Could not Create series", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			//--------------------------------------------------------------------//
			//  Step-10: verify sort Program group in record attendance			  //
			//--------------------------------------------------------------------//
			sortType = "by name";
			assertTrue(selectValueFromAjaxList(selenium,ajxAttendanceOrderTypesuggestBox,sortType ),"Could not select based on sort type", selenium, ClassName, MethodName);
		    waitForPageLoad(selenium);
		    
			assertTrue(verifyStoredValuesForFirstRecordAttendance(selenium,pdgSeriesData1, pDGData1),"Could not verify the Created series in record attendance", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			assertTrue(verifyStoredValuesForFirstRecordAttendance(selenium,pdgSeriesData, pDGData),"Could not verify the Created series in record attendance", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(verifyStoredValuesForRecordAttendanceSummary(selenium,pdgSeriesData1, pDGData1),"Could not verify the Created series in record attendance", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
