...
public class VerifyPatientAlertInPatientSearch extends AbstractOperationsTest {
    @Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify patient alert in wait list management")
	...
	***************************************************************************************************/
	...
	public boolean waitListManagement(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, OperationsTestLib addWaitList ) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			assertTrue(click(selenium, btnAddPatientButton), "Could not click the Add new wait list", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			//--------------------------------------------------------------------//
			//  Step-6: Advanced search with Patient ID							  //
			//--------------------------------------------------------------------//
			assertTrue(searchPatientForBookingVisit(selenium,addWaitList.patientName), "Could not search patient for Booking Visit", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			selenium.keyPress(txtPatientBox, "\\13");
			...
			assertTrue(getText(selenium,lblPatientResult).toLowerCase(new java.util.Locale("en","US")).trim().contains(addWaitList.patientName.trim().toLowerCase(new java.util.Locale("en","US"))),
			...
			//--------------------------------------------------------------------//
			assertTrue(getText(selenium,lblPatientAlertArea).toLowerCase(new java.util.Locale("en","US")).trim().contains(addWaitList.comment.trim().toLowerCase(new java.util.Locale("en","US"))), 
					"Not able to find text " +addWaitList.comment , selenium, ClassName, MethodName);						
