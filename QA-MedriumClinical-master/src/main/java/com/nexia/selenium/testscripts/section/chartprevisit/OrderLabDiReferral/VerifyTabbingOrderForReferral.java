...
public class VerifyTabbingOrderForReferral extends AbstractChartPreVisit{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for ordering Labs")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyTabbongOrderForReferral(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	public void verifyTabbingOrder(String seleniumHost,int seleniumPort, String browser, String webSite, String userAccount,ChartPreVisitLib orderData) throws InterruptedException, IOException{
		...
		try{
			String csspaths[]={ajxSearchReferral, chkboxUrgent, txtAuthcode, ajxNotifyDuration, ajxDurationUnit};
			...
			waitForPageLoad(selenium);
			selectValueFromAjaxList(selenium, ajxSearchReferral,orderData.referral1);
			waitForPageLoad(selenium);
			...
			for(int i =0;i<csspaths.length;i++){
				...
				if(csspaths[i].equals(chkboxUrgent)){
					selenium.keyPress(csspaths[i], "\\9");
				}
			}
