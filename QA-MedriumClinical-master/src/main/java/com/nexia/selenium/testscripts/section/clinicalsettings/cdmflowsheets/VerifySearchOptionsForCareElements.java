...
public class VerifySearchOptionsForCareElements extends AbstractCdmFlowSheets{
	@Test(groups = {"SingleThread","Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test to verify various search options for care elements in patient chart.")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySearchOptionsForCareElementsInPatientChart(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClinicalSettingLib mcTemplate = new ClinicalSettingLib();
		mcTemplate.workSheetName = "ManagedCaredTemplate";
		mcTemplate.testCaseId = "TC_MCT_027";
		mcTemplate.fetchClinicalSettingsTestData();
		
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
	
		verifySearchOptionsForCareElements(seleniumHost, seleniumPort, browser, webSite, userAccount, mcTemplate);
	}
	
	/*************************************************************************************************** 
	* @purpose        To verify various search options for care elements in patient chart.
	* @Specification  SRS_EMR_CDM_Flowsheets Rev0.0
	* @action 		  Verifying search options for care elements in patient chart
	* @author         AspireQA
	* @throws 		  IOException 
	* @state          Developing
	* @useraccount    Both
	...
	***************************************************************************************************/
	...
	public boolean verifySearchOptionsForCareElements(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalSettingLib mcTemplate) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-7: Navigate to Managed Care in Patient Chart//
			//--------------------------------------------------------------------//
			...
			assertTrue(click(selenium,btnViewFullWidth),"could ot click btnViewFullWidth", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);

			assertTrue(isTextPresent(selenium,"View split screen"),"Care Element is present", selenium, ClassName, MethodName);
			
			assertTrue(click(selenium,btnViewFullWidth),"could ot click btnViewFullWidth", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			//--------------------------------------------------------------------//
			//  Step-8: Verify the search option//
			//--------------------------------------------------------------------//
			assertTrue(type(selenium,txtSearchCareElement,mcTemplate.display),"Could not type", selenium, ClassName, MethodName);

			assertTrue(clickAt(selenium,txtSearchCareElement,""),"Could not click", selenium, ClassName, MethodName);
			assertTrue(focus(selenium,txtSearchCareElement),"Could not Focus", selenium, ClassName, MethodName);
			assertTrue(keyPress(selenium,txtSearchCareElement, "\\13"),"Could not KeyPress", selenium, ClassName, MethodName);

			assertTrue(isTextPresent(selenium,mcTemplate.display),"Care Element is not present", selenium, ClassName, MethodName);
			
			assertTrue(!isTextPresent(selenium,"dsd"),"Care Element is present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
