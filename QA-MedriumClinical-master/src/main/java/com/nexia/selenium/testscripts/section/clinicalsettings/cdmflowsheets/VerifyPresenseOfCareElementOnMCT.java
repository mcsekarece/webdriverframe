...
public class VerifyPresenseOfCareElementOnMCT extends AbstractCdmFlowSheets{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test to verify care element added in patient chart is not present in managed care template")
	...
	***************************************************************************************************/
	...
	public boolean verifyPatientChartCareElementNotPresentInManageCareTemplate(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalSettingLib mcTemplate) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			assertTrue(click(selenium,lnkMore),"Could not click the More link;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);

			assertTrue(click(selenium,lnkFlowSheets),"Could not click the flow sheets link;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,btnAddNew),"Could not click the Add New;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			...
			assertTrue(selectValueFromAjaxList(selenium, ajxNameMedication,mcTemplate.templateName+uniqueName),"Could not choose medication;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(enterDate(selenium, txtBaseLineDate, date),"Could not type date;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,btnSaveWhole),"Could not click Save;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,btnViewEdit),"Could not click the View Edit;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
