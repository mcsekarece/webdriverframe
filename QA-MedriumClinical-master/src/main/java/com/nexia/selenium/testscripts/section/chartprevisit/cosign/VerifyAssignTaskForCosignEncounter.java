...
public class VerifyAssignTaskForCosignEncounter extends AbstractChartPreVisit{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verifing Asign task of  Co-signed encounter in  encounter Queue ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyAssignTaskForCosignEncounters(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
...
try
{
		...
		assertTrue(loginForNexiaFromProviderSite(selenium, userAccount, encounterData.userName, encounterData.userPassword),"Login Failed ",selenium,ClassName,MethodName);
			...
			returnValue=true;
				...
				waitForPageLoad(selenium);
			//	waitForPageLoad(selenium);	
				assertTrue(click(selenium,lnkEncounterAction),"Could not click the Action button",selenium,ClassName,MethodName);    
				waitForPageLoad(selenium);		
				assertTrue(click(selenium,"//a[@id='assignTask']/span"),"Could not click the Assign task",selenium,ClassName,MethodName);      
				waitForPageLoad(selenium);		
			//	assertTrue(isElementPresent(selenium,"css=div.chart-title.title-underlined"),"Encounter Note Not Present After Click Task link",selenium,ClassName,MethodName);      
				
				//--------------------------------------------------------------------//
				//  Step-10: create task											  //
				//--------------------------------------------------------------------//
				ChartPreVisitLib taskData = new ChartPreVisitLib();
				taskData.workSheetName = "CreatePatientTask";
				taskData.testCaseId = "TC_CD_003";
				taskData.fetchChartPreVisitTestData();
				assertTrue(createPatientTaskForCosign(selenium,taskData),"Create task failed",selenium,ClassName,MethodName);    
			//	assertTrue(isElementPresent(selenium,"css=div.chart-title.title-underlined"),"Encounter Note Not Present After Click save button in  Task page",selenium,ClassName,MethodName);      
