...
public class VerifySaveSearch  extends AbstractClinicalList  {
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for verify Save Search Text For Male ")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySaveSearchTextForMale(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for verify Save Search With Contact Checked")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySaveSearchWithContactChecked(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		ClinicalListLib searchData = new ClinicalListLib();
		searchData.workSheetName = "verifySearchForSex";
	...
	public boolean verifySex(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalListLib searchData) throws IOException{
		...
		try{
			...
			if(searchData.testCaseId.equals("TC_VSS_002")){
				assertTrue(click(selenium,chkContact),"Could not click on contact check box;More Details", selenium, ClassName, MethodName);
				waitForPageLoad(selenium);
			}
			assertTrue(click(selenium,btnSave),"Could not click on Save button button;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			waitForPageLoad(selenium);
			assertTrue(type(selenium,txtClinicalListSave,searchData.searchName),"Could not enter the commend;More Details:"+searchData.toString(), selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			Calendar cal=Calendar.getInstance();
			SimpleDateFormat DateFormat=new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
			String uniqueName=DateFormat.format(cal.getTime());
			assertTrue(type(selenium,txtClinicalListSave,uniqueName+uniqueName),"Could not enter the commend;More Details:"+searchData.toString(), selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			assertTrue(click(selenium,btnSaveBtn),"Could not click on Save button button;More Details", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			waitForPageLoad(selenium);
			isElementPresent(selenium,lblSavedSearchInSummary);
			if(selenium.isConfirmationPresent()){
				selenium.chooseOkOnNextConfirmation();
			}
			assertTrue(verifySavedSearch(selenium,searchData,uniqueName),"Patient Checkin Failed", selenium, ClassName, MethodName);
	 ...
	 * @since	Nov 22,2012
	 */
	public boolean verifySavedSearch(Selenium selenium,ClinicalListLib searchData,String uniqueName) throws IOException{
		assertTrue(click(selenium,lnkClosedArrow),"unable to click the arrow",selenium,ClassName,MethodName);
		assertTrue(isTextPresent(selenium,uniqueName.trim())," ", selenium, ClassName, MethodName);
		return true;
	}
}
