...
public class CreateNewAdminPractice extends AbstractSystemSettingsTest{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Adding a Practice")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void createNewPractices(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		createPractices(seleniumHost, seleniumPort, browser, webSite, userAccount, pracData);
	}
	
	@Test(groups = {"AdvancedSmoke","Smoke","Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Adding a Practice With Mandatory")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void createNewPracticesWithMandatory(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		SystemSettingsLib pracData = new SystemSettingsLib();
		pracData.workSheetName = "CreatePractice";
		...
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		createPracticesWithMandatory(seleniumHost, seleniumPort, browser, webSite, userAccount, pracData);
	}
	 ...
	 * @since  	    July 30, 2012
	 */
	public boolean createPractices(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SystemSettingsLib pracData) throws Exception{
	 ...
	 * @since  	    July 31, 2012
	 */
	public boolean createPracticesWithMandatory(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SystemSettingsLib pracData) throws Exception{
			...
			try{
				...
				//--------------------------------------------------------------------//
				if(!verifyStoredValuesForMandatory(selenium,pracData,userAccount)){
					Assert.fail("Practice details not saved properly; More Details :"+ pracData.toString());
					returnValue=false;
				}
				assertTrue(click(selenium,btnDonePratice),"Could not click on done button:" + pracData .toString(), selenium, ClassName, MethodName);
	...
	public boolean verifyStoredValuesForMandatory(Selenium selenium,SystemSettingsLib pracData,String account) throws IOException{
		...
		if(isElementVisible(selenium, btnTaxId)) { 
			if(!getValue(selenium,btnTaxId).toLowerCase(new java.util.Locale("en","US")).trim().contains(pracData.taxId.trim().toLowerCase(new java.util.Locale("en","US"))))
				return false;
		}
		if(!getValue(selenium,txtAddr1).toLowerCase(new java.util.Locale("en","US")).trim().contains(pracData.address1.trim().toLowerCase(new java.util.Locale("en","US")))){
