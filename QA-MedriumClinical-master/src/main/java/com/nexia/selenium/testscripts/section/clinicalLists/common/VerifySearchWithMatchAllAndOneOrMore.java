...
public class VerifySearchWithMatchAllAndOneOrMore extends AbstractClinicalList {
	...
	public boolean verifySearch(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalListLib ageData) throws Exception{
		...
		try{
			...
			assertTrue(type(selenium, String.valueOf(possition)+"anddurationIntegerMax", ageData.year1), "Could not enter minimum year", selenium, ClassName, MethodName);
			possition = possition+1;
			assertTrue(click(selenium, btnAddNewCriteria), "Could not click on Add New button for Match All Filter", selenium, ClassName, MethodName);
			selectValueFromAjaxList(selenium,String.valueOf(possition)+"andsearchTypeSuggestBoxsuggestBoxsuggestBox",ageData.searchType2);
			...
			}catch(Exception e){anddurationIntegerMaxTextBox
				selenium.focus(btnNewSearch);
				selenium.clickAt(btnNewSearch, "");
			}*/
