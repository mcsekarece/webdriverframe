...
public class VerifyDoNotIncludeForSex extends AbstractClinicalList {
	...
	public boolean verifySex(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalListLib searchData) throws IOException{
		...
		try{
			...
			waitForPageLoad(selenium);
			assertTrue(runSearchQueryForSexWithDonNotInclude(selenium,searchData,possition),"Creation failed", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			waitForPageLoad(selenium);
			//--------------------------------------------------------------------//
			...
			assertTrue(click(selenium, btnNewSearch), "Could not click on Clinical Lists", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			assertTrue(click(selenium, btnAddForMatchAll), "Could not click on Clinical Lists", selenium, ClassName, MethodName);
			...
			else if(noOfRecords==noOfRecords1){
				return returnValue;
			}
