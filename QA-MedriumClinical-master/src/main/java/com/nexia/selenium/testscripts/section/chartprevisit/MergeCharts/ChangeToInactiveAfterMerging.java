...
public class ChangeToInactiveAfterMerging extends AbstractHomeTest{
	...
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	 public void changeToInactiveAfterMerging(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
			...
			changeActiveToInactive(seleniumHost, seleniumPort, browser, webSite, userAccount, patientData);
		}
public boolean changeActiveToInactive(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLib patientData) throws IOException
{
	...
	try{
		...
		waitForPageLoad(selenium);
		//----------------------------------------------------------------//
		//  Step-9: Change patient Status as Inactive
		//----------------------------------------------------------------//
		assertTrue(navigateToPatientDemographicsPage(selenium,patientId2),"Unable to navigate to demographics page",selenium,ClassName,MethodName);
		HomeLib patientData2 = new HomeLib();
		patientData2.workSheetName = "PatientData";
		patientData2.testCaseId = "TC_NPC_152";
		patientData2.fetchHomeTestData();
		waitForPageLoad(selenium);
		assertTrue(click(selenium,lnkPatientInfoView),"unable to click the patient info view link",selenium,ClassName,MethodName);
		waitForPageLoad(selenium);
		assertTrue(click(selenium,btnRegistrationEdit),"unable to click the edit button",selenium,ClassName,MethodName);
		waitForPageLoad(selenium);
		assertTrue(selectValueFromAjaxList(selenium, ajxStationInactivation, patientData2.reasonForInactivation),"Unable to enter the reason for inactivation",selenium,ClassName,MethodName);
		waitForPageLoad(selenium);
		assertTrue(click(selenium,btnSave1),"Unable to click the save button",selenium,ClassName,MethodName);
		waitForPageLoad(selenium);
		assertTrue(waitForElement(selenium, btnRegistrationEdit, 10000),"Could not find the edit button", selenium, ClassName, MethodName);
