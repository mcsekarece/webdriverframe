...
public class VerifyHideOptionInSocialHistoryFromPatientSummary extends AbstractChartPreVisit{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Hide Option In Social History From Patient Summary Page")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyHideOptionInSoicalHisitory(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	/*************************************************************************************************** 
	* @purpose        To verify Hide option in Medication From Social History Summary Page
	* @action 		  verifying Hide option in Medication From Social History summary page 
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	public boolean verifyHideOption(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount,ChartPreVisitLib customiszeData) throws IOException{
		...
		try{
			...
			if(!isChecked(selenium,chkSocialHistoryOptionalElement))
				assertTrue(click(selenium,chkSocialHistoryOptionalElement),"Could not click the Edit button", selenium, ClassName, MethodName);
			
			String uncheckElementName = getText(selenium,lblSocialHistoryOptionalElement);
			waitForPageLoad(selenium);
			...
			//--------------------------------------------------------------------//
			assertTrue(checkOptionalElement(selenium, sectionName, chkSocialHistoryOptionalElement), "Could not Navigate the Patient Summary", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
