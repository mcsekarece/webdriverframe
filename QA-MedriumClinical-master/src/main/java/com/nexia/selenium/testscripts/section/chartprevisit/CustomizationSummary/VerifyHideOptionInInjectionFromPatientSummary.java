...
public class VerifyHideOptionInInjectionFromPatientSummary extends AbstractChartPreVisit{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Hide Option In njection From Patient Summary Page")
	...
	/*************************************************************************************************** 
	* @purpose        To verify Hide option in Injection From Patient Summary Page
	* @action 		  verifying Hide option in Injection From Patient summary page 
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	public boolean verifyHideOption(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount,ChartPreVisitLib customiszeData) throws IOException{
		...
		try{
			...
			if(isChecked(selenium,chkInjectionOptionalElement))
				assertTrue(click(selenium,chkInjectionOptionalElement),"Could not click the Edit button", selenium, ClassName, MethodName);
			...
			//--------------------------------------------------------------------//
			assertTrue(checkOptionalElement(selenium, sectionName1, chkInjectionOptionalElement), "Could not Navigate the Patient Summary", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
