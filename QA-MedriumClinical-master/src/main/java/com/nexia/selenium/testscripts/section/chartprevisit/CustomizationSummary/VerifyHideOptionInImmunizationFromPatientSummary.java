...
public class VerifyHideOptionInImmunizationFromPatientSummary extends AbstractChartPreVisit{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verify Hide Option In Immunization From Patient Summary Page")
	...
	/*************************************************************************************************** 
	* @purpose        To verify Hide option in Immunization From Patient Summary Page
	* @action 		  verifying Hide option in Immunization From Patient summary page 
	* @author         Aspire QA
	* @throws         IOException 
	* @state          Developing
	...
	public boolean verifyHideOption(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount,ChartPreVisitLib customiszeData) throws IOException{
		...
		try{
			...
			if(isChecked(selenium,chkImmuizationOptionalElement))
				assertTrue(click(selenium,chkImmuizationOptionalElement),"Could not click the Edit button", selenium, ClassName, MethodName);
			...
			//--------------------------------------------------------------------//
			assertTrue(checkOptionalElement(selenium, sectionName1, chkImmuizationOptionalElement), "Could not Navigate the Patient Summary", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
