...
public class VerifySaveInClaimAdjustment extends AbstractBilling {
    	
    @Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Verify save in claim adjustment")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifySaveInClaimAdjustment(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		verifySaveInClaimAdjustment(seleniumHost, seleniumPort, browser, webSite, userAccount, billingData);
	}

    /*************************************************************************************************** 
	* @purpose        Verify save in claim adjustment
	* @action 		  verifying save in claim adjustment
	...
	* @since  	      Feb 06, 2014	
	***************************************************************************************************/
	public boolean verifySaveInClaimAdjustment(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, BillingLib billingData) throws IOException{
		...
		try{
			...
			assertTrue(click(selenium, CASSave), "Could not click save button in CAS", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			//--------------------------------------------------------------------//
			//  Step-5: Verify save in claim adjustment							  //
			//--------------------------------------------------------------------//
			assertTrue(click(selenium, btnShowClaimAdjustment), "Could not click show claim adjustment", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			
			if (!((getValue(selenium, ajxGroupCode ).trim().toLowerCase()).contains(billingData.groupCode.trim().toLowerCase()))) {
				return false;
			}
			if (!((getValue(selenium, ajxReasonCode).trim().toLowerCase()).contains(billingData.reasonCode.trim().toLowerCase()))) {
				return false;
			}
			if (!((getValue(selenium, txtCASAmount).trim().toLowerCase()).contains(billingData.CASAmount.trim().toLowerCase()))) {
				return false;
			}
