...
public class CreateNewPatientAndPayerOne extends AbstractHomeTest{
	
	public String workBookName = "TestData_Home.xls";		
	public String sectionName = "home";
	
	
	@Test(groups = {"EndToEnd","Developing","firefox", "iexplore", "safari", "default","BeforeCreatePatientAndPayer" }, description = "Test for Adding a New patient and Medical Payer")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void createNewpatientDataWith018(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount)throws Exception{
	...
	***************************************************************************************************/
	public boolean createNewPatientWithMandatoryFields (String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, HomeLib patientData) throws Exception{
		...
		try{
			...
			payerData2.testCaseId = "TC_CP_025";
			rowPossition = 25;
			wD.writeDataToExcel(sectionName, workBookName,payerData.workSheetName, rowPossition, columPossition, patientData.lastName);
			...
			//Create Third Medical Payer
			
				HomeLib payerData3 = new HomeLib();
				payerData3.workSheetName = "AddPayers";
				payerData3.testCaseId = "TC_CP_026";
				rowPossition = 26;
				wD.writeDataToExcel(sectionName, workBookName,payerData.workSheetName, 26, columPossition, patientData.lastName);
				payerData3.fetchHomeTestData();
				
				
				if(!selenium.isTextPresent(payerData3.payerName)){
					assertTrue(createEndToEndPayerList(selenium,payerData3,date,userAccount),"Add New Payer failed",selenium, ClassName, MethodName);
					waitForPageLoad(selenium);
					if(!verifyPayerDetails(selenium,payerData3 )){
						Assert.fail("Payer Details are not saved Properly");
					}else
						returnValue = true;
				}			
