...
public class VerifyPaperCliamStatus extends AbstractSystemSettingsTest{
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Paper a Associate Payer")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void paperClaimsPayer(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		verifyPaperCliam(seleniumHost, seleniumPort, browser, webSite, userAccount, providerData);
	}
	/**
	 * verifyPaperCliam
	 * function to verify Paper Cliam
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	 ...
	 * @since  	    Feb 22, 2014
	 */
	public boolean verifyPaperCliam(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, SystemSettingsLib providerData) throws Exception{
		...
		try{
			...
			// --------------------------------------------------------------------//
			// Step-16: Naviage to Cliam Queue
			// --------------------------------------------------------------------//
			assertTrue(click(selenium,billingData.lnkPaperClaims),"Could not click the Paper Claims Queue",selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			// --------------------------------------------------------------------//
			// Step-17: Search Cliam Details
			// --------------------------------------------------------------------//
			...
			// --------------------------------------------------------------------//
			// Step-18: Verify Claims Details
			// --------------------------------------------------------------------//
			if(!verifyPaperClaimsDetails(selenium, postChargeEntry,payerData,providerPayerData, providerData)){
				Assert.fail("Paper Claims Details are not saved");
			}else
				returnValue = true;
	...
	public boolean verifyPaperClaimsDetails(Selenium selenium,BillingLib postChargeEntry,HomeLib payerData,SystemSettingsLib providerPayerData,SystemSettingsLib providerData ){
		AbstractBilling billingData = new AbstractBilling();
		
		selectValueFromAjaxList(selenium, billingData.ajxDateFor,"Last 30 days");
		waitForPageLoad(selenium);
		int count;
		...
		for(count =1; count <=claimsCount;count++ ){
			if((getText(selenium,"//div[4]/div/div/div["+count+"]/div").contains(payerData.patientId))
				...
				if(!getText(selenium,"//div[4]/div/div/div["+count+"]/div").contains(postChargeEntry.toDate)){
					return false;
				}
