...
public class ValidateMammogramRecallListEligibility extends AbstractChartPreVisit {

	@Test(groups = {"Regression", "Approved","firefox", "iexplore", "safari", "default" }, description = "Test to validate Mammogram recall list eligibility")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void mammogramRecallListEligibilityTest(String seleniumHost, int seleniumPort, String browser, String webSite, String userAccount) throws Exception {
		ChartPreVisitLib careData = new ChartPreVisitLib();
		careData.workSheetName = "PreventiveCare";
		careData.testCaseId = "TC_PC_012";  	//TODO Create a patient from portability script. Not done due to env issue
		careData.fetchChartPreVisitTestData();
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();

		verifyEligibilityForMammogramRecallList(seleniumHost, seleniumPort, browser, webSite, userAccount,careData);
	}

	/*************************************************************************************************** 
	 * @purpose        Test to validate Mammogram recall list eligibility
	...
	public boolean verifyEligibilityForMammogramRecallList(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib careData ) throws IOException{
		...
		try{
			...
			//Noticed that Automatic Alerts are not reflected immediately, hence setting them manually 
			//--------------------------------------------------------------------//
			//  Step-3: Set Manually Care Alert for the patient  										
			//--------------------------------------------------------------------//
			assertTrue(setForceRecallAlertForPatient(selenium,userAccount,careData), "Could not set Force Recall for patient", selenium, ClassName, MethodName);
	...
	private boolean setForceRecallAlertForPatient(Selenium selenium, String userAccount, ChartPreVisitLib careData) throws IOException {
