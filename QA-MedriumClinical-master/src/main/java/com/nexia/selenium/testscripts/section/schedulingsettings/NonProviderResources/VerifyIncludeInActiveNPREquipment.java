...
public class VerifyIncludeInActiveNPREquipment extends AbstractSchedulingTest {
		...
		try{
			...
			for(int count=1;count<=resourceNPREquipCount;count++){		
			 ...
			 if(getText(selenium, lblSummaryPageNonProviderResourceRoom).contains(nonProviderEq.equipmentName)){
			  ...
			  waitForPageLoad(selenium);
			  assertTrue(selectValueFromAjaxList(selenium,txtSearchNonProviderResource, nonProviderEq.equipmentName+uniqueName),"", selenium, ClassName, MethodName);
			  
			assertTrue(getText(selenium, lblSummaryPageNonProviderResourceRoom).contains(nonProviderEq.equipmentName),"The Include inactive check box not working properly", selenium, ClassName, MethodName);
