...
public class UnitTestForSuperBillInformationPage extends AbstractBilling{
	@Test(groups = {"RegressionUnit","firefox", "iexplore", "safari", "default" }, description = "Template with empty")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void templateWithEmpty(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		...
		superBillData.testCaseId = "TC_USBI_001";
		superBillData.fetchBillingTestData();
		unitTestForSBI(seleniumHost, seleniumPort, browser, webSite, userAccount, superBillData);
	}
	
	@Test(groups = {"RegressionUnit","firefox", "iexplore", "safari", "default" }, description = "Template with More Char")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void templateWithMoreChar(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
		ClassName=this.getClass().getName().substring(39);	
		MethodName= Thread.currentThread().getStackTrace()[1].getMethodName();
		BillingUnitTestLib superBillData = new BillingUnitTestLib();
		superBillData.workSheetName = "UnitTest_AdminSuperBill";
		superBillData.testCaseId = "TC_USBI_002";
		superBillData.fetchBillingTestData();
		unitTestForSBI(seleniumHost, seleniumPort, browser, webSite, userAccount, superBillData);
	}
	...
	public boolean unitTestForSBI(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, BillingUnitTestLib superBillData) throws IOException{
		...
		try {
			...
			if(!superBillData.alert.equals("")){
			    ...
			    if(superBillData.testCaseId.equals("TC_USBI_002")){
					if(getValue(selenium,txtTempletaName).toLowerCase(new java.util.Locale("en","US")).trim().contains(superBillData.templateName.trim().toLowerCase(new java.util.Locale("en","US")))){
					       return false;
					}
			    }
			}
