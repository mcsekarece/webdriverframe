...
public class VerifyDisplayAsInEncounterTemplate extends AbstractClinicalSettings {
	
	@Test(groups = {"SingleThread","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Verifing display as in encounter template")
	@Parameters( { "seleniumHost", "seleniumPort", "browser", "webSite", "userAccount"})
	public void verifyDisplayAsInEncounterTemplate(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount) throws Exception {
	...
	/*************************************************************************************************** 
	* @purpose        To verify display as in encounter template
	* @Specification  SRS_EMR_Encounter_Structure_Templates Rev0.0
	* @action 		  Verifying display as in encounter template
	* @author         AspireQA
	* @throws 		  IOException 
	* @state          Developing
	* @useraccount    Both
	...
	***************************************************************************************************/
	...
	public boolean encounterTemplate(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ClinicalSettingLib encounterTemplateTestData) throws IOException{
		...
		try{
			...
			//--------------------------------------------------------------------//
			//  Step-6: verify display as name in summary page     			      //
			//--------------------------------------------------------------------//
			...
			waitForPageLoad(selenium);
			assertTrue(selenium.isTextPresent(encounterTemplateTestData.customSection1),"Could not fid the text", selenium, ClassName, MethodName);
