...
public class CancelChargeEntry extends AbstractBilling {
	...
	@Parameters({ "seleniumHost", "seleniumPort", "browser", "webSite",
			"userAccount" })
	public void cancelChargeEntry(String seleniumHost, int seleniumPort,
		...
		chargeEntryData.testCaseId = "TC_CE_010";
		chargeEntryData.fetchBillingTestData();
		cancelChargeEntry(seleniumHost, seleniumPort, browser, webSite,
				userAccount, chargeEntryData);
	}

	public boolean cancelChargeEntry(String seleniumHost, int seleniumPort,
			String browser, String webSite, String userAccount,
		...
		try {
			...
			if (chargeEntryData.testCaseId.equals("TC_CE_009")) {
				...
				assertTrue(isElementPresent(selenium, txtServiceLineUnits1),
						"Action Button not present", selenium, ClassName,
						MethodName);
				assertTrue(
						type(selenium, btnActions,
								chargeEntryData.unitsServiceLine),
						"not able to click Action Button", selenium, ClassName,
						MethodName);
				assertTrue(isElementPresent(selenium, lnkCancel),
						"Cancel Link not present", selenium, ClassName,
						MethodName);
				assertTrue(click(selenium, lnkCancel),
						"Not able to Click cancel link", selenium, ClassName,
						MethodName);
				assertTrue(isElementPresent(selenium, btnNexiaLogo),
						"Did not Redirect to home Page", selenium, ClassName,
						MethodName);
			}
