...
public class VerifyUnmatchedRecordOnRefile extends AbstractChartPreVisit {
	...
	public boolean verifyLabReport(String seleniumHost, int seleniumPort,String browser, String webSite, String userAccount, ChartPreVisitLib orderData) throws IOException{
		...
		try{
			...
			assertTrue(click(selenium,lnkRefileOption),"Could not click the btn Refile", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,chkReturnToUnMatch),"Could not click the File to Patient link", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,btnSearchSavePatient),"Could not click the save patient", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(selenium.isTextPresent("Unmatched"),"Text is not present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,btnFile),"Text is not present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(selenium.isTextPresent("Possible Matches"),"Text is not present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(selenium.isTextPresent("Match"),"Text is not present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(selenium.isTextPresent("Match and Update"),"Text is not present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(click(selenium,btnMatch),"Could not click the match patient", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			
			assertTrue(selenium.isTextPresent("Are you sure you want to match the patient to this lab report?"),"Text is not present", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);
			assertTrue(click(selenium,btnYes),"Could not click the match patient", selenium, ClassName, MethodName);
			waitForPageLoad(selenium);			
			arrowPossition = selenium.getAttribute("css=a.arrow-closed/@class");
			if(arrowPossition.equalsIgnoreCase("arrow-closed")){
				 assertTrue(click(selenium,"css=a.arrow-closed"),"Could not click the expand arrow of search Patient", selenium, ClassName, MethodName);
	...
	public boolean searchFileToPatient(Selenium selenium) throws IOException{
		...
		waitForElement(selenium,txtSearchFileToPatient,10000);
