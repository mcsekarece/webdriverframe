...
public class UpdatePreferredDrugList extends AbstractPreferenceTest{

	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for Updating the preferred drug list")
	...
	public void verifyCreatePreferredDrugList(String seleniumHost, int seleniumPort,String browser, String webSite){
		...
		drugListTestData.workSheetName = "UpdatePreferredDrugList";
		drugListTestData.testCaseId = "TC_UDRU_001";		
	...
	/**
	 * verifyCreatePreferredDrugList
	 * function to verify Create  Preferred DrugList
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
		...
		try{
			...
			while(selenium.isElementPresent( "productTargetBox"+counter)){ 
				if(getText(selenium, "productTargetBox"+counter).contains(drugListTestData.medicationName)){
					isRecordFound = true;
					if(getText(selenium, "productReplacementSuggestBox"+counter).contains(drugListTestData.altMedicationName)){
						drugListTestData.altMedicationName = drugListTestData.updateMedicationName;
			...
			click(selenium, "//div[@id='productReplacementSuggestBox"+preferenceCounter+"']/div/div/div/div/ul/li/span[2]");
			Assert.assertTrue(selectValueFromAjaxList(selenium,"productReplacementSuggestBox"+preferenceCounter+"Box", drugListTestData.altMedicationName),"Could not enter Alt Medication Name; More Details" +drugListTestData.toString());
			searchPatient(selenium,drugListTestData.patientID);
