...
public class VerifyClinicalQualityMeasure extends AbstractPreferenceTest {
	...
	@Test(groups = {"Regression","Approved","firefox", "iexplore", "safari", "default" }, description = "Test for verifying clinical Quality measure")
	...
	/**
	 * validateClinicalQualityMeasure
	 * function to validate Clinical Quality Measure
	 * @param 		seleniumHost
	 * @param 		seleniumPort
	 * @param 		browser
	 * @param 		webSite
	...
	public void validateClinicalQualityMeasure(String seleniumHost, int seleniumPort,String browser, String webSite){
		...
		try{
			...
			selenium.select("id=providersInput","label=Juanmanuel Pancrasio");
			...
			else if(selenium.isChecked("0013"))
			{
				selenium.click("id=qualityMeasureEditSave");
				waitForPageLoad(selenium);
			}
